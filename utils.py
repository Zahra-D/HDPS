...
# Based only on last working year 
...
# Based on 10 year gaps before T_ER

# Return work_retirement instead of work if t > T_ER
...
def cal_regu_term_each10(model): # Only regulare general layer 1
  ...
  for year in range(AGE_0,T_LR):
    ...
    if (((year-1)%10) == 0) or (year == 69): # Skips every 10th year or the last year
      continue
    pin_year = (((year-1)//10)+1)*10 +1 # Finds the closest previous 10-year anchor
    if year >= 62:
      # retier_year = 1
      pin_year = 69
...
def cal_regu_term_each10_old(model): # Regularize all layers
...
def save_checkpoint(model, optimizer, base_dir, epoch):
    ...
    pathlib.Path(f'{base_dir}/epoch{epoch}').mkdir(parents=True) # Folder of a run and epoch
