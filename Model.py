...
class Working_Model(nn.Module):  # Defines a neural network model for working years of the agent
    ...
    def __init__(self, SS_Type, SS_Param=None, d_model=64, num_heads=4, dropout=0.0, y_gate_temp=10.0, soft_rank_temp=0.1):  # Initialize model with architecture and economic parameters
        super().__init__()  # Call the parent constructor from nn.Module

        self.alpha_x = nn.Parameter(torch.tensor(0.0), requires_grad=False)  # Start from 0, learnable (scale of res)
        self.alpha_h = nn.Parameter(torch.tensor(0.0), requires_grad=False)
        ...
        self.soft_rank_temp = soft_rank_temp # Temperute used for soft ranking of yearly incomes
        ...
        nn.init.constant_(self.task_layer_h_core[-1].bias, torch.logit(torch.tensor(H_FT/h_max)))

        # Core task head biases for consumption share (target ~50% for workers)
        nn.init.constant_(self.task_layer_x_core[-1].bias, torch.logit(torch.tensor(0.01)))
    ...
    def forward(self, y_seq, static_inputs, year, tau=None):  # Forward pass: compute h and x
        ...
        if self.SS_Type == 'Life_Time':  # If using average of income
            ...
            rank = self.soft_rank(y_seq, tau=self.soft_rank_temp)  # Compute soft rank
            ...
            curr_temp = tau if tau is not None else self.y_gate_temp # Dynmaic temp
            gate = torch.sigmoid((rank - threshold) * curr_temp).to(y_seq.device) # Gate signal based on threshold
        ...
        h_sum = h_core + self.alpha_h*h_res
        h_nor = (h_sum - h_sum.mean()) / (h_sum.std() + 1e-5)
        # Sigmoid last layer model
        #h_out = torch.sigmoid(h_nor)  # Total h
        # Check if we are in bilevel or staged_core_then_res mode and haven't unfrozen h_core yet
        # training_args = getattr(self, 'training_args', None)
        # if training_args and training_args.transfer_learning_type in ['bilevel_freeze_res', 'staged_core_then_res']:
        #     if not self.task_layer_h_core[0].weight.requires_grad:  # Check if still frozen
        ...
        x_sum = x_core + self.alpha_x *x_res
...
class Retiree_Model(nn.Module):  # Model that simulates retirement behavior based on savings and pension
    ...
    def __init__(self, num_hidden_units_R=5, activation_function=nn.GELU):  # Constructor: initializes model components
        ...
        nn.init.constant_(self.core[-1].bias, torch.logit(torch.tensor(0.5)))
        for res in self.res:
...
class Master_Model(nn.Module):  # Combines Working_Model and Retiree_Model for full life-cycle simulation
    ...
    def simulate(self, theta_sim_t, edu_sim, a_sim_0, tau=None):  # Run simulation of lifecycle using working and retirement models
        ...
        for t in range(T_W):  # Loop over working years
            ...
            h_out, x_out = self.working_model(y_input, static_inputs, t, tau=tau)  # Predict h and x using working model
