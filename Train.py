...
# Transfer Learning
parser.add_argument("--transfer_learning_type", type=str, choices=["none", "plain", "bilevel", "warm", "freeze_res"], default="none")
...
def train_step(model, dataloader, epoch, writer, optimizer):
    ...
    for batch in iterator:
        ...
        out = model.simulate(theta.to(device), edu.to(device), a0)
        c_t = out["c_sim_t"]
        h_t = out["h_sim_t"]
...
# Save final
save_checkpoint(model, optimizer, model_dir, args.num_epochs - 1)
