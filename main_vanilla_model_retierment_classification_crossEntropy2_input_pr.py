...
from model_retirement_classification_input_pr import Model_retirement
...
def train_step(model, dataloader, epoch, s_writer, optimizer, device, args):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        optimizer.zero_grad()
        pr, c_w, c_wr, c_r, a_w, a_wr, a_r, all_h, all_y = model(theta_t_.to(device), edu_.to(device), a_1.to(device),w_t_)
        
        c_t = torch.concat([c_w, (1-pr) * c_wr[:, :, 0] + pr * c_wr[:,:, 1], c_r ], dim = -1)

        loss = loss_function_retirement_pr_cross(model, pr, c_t,  all_h, epoch, s_writer, args)
...
def main(args):
    ...
    # dataset_eval = generating_dataset(J,  T_LR-AGE_0, THETA_0, P_EDU)
    ...
    for lr in lrs:
        ...
        if (args.num_hidden_unit == 0):
            num_h_u_s = [30,10]
        else:
            num_h_u_s = [args.num_hidden_unit]
            
            
            
            
            
        for num_h_u in num_h_u_s:
            
            model = Model_retirement(num_hidden_node=num_h_u)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_hidden_unit', default=10, type=int, help='number of hidden units for the network')
